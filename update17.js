(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .rt-recording-dot{width:10px;height:10px;border-radius:50%;background:#5fd0ff;box-shadow:0 0 0 0 rgba(95,208,255,.65)}.rt-recording-banner.verified-recording .rt-recording-dot{background:#ff405d;animation:rtPulse 1.4s infinite}.rt-recording-banner.starting .rt-recording-dot{background:#ffd166;animation:rtPulse 1.4s infinite}.rt-recording-copy{min-width:0}.rt-recording-copy strong{display:block}.rt-recording-copy span{display:block;font-size:12px;opacity:.82;line-height:1.35}.rt-recording-actions{display:flex;align-items:center;gap:7px}.rt-recording-actions button{border:1px solid #47708e;border-radius:12px;padding:10px 12px;background:#102b40;color:#fff;font-weight:850}.rt-recording-auto{background:#5fd0ff!important;color:#001522!important;border-color:#5fd0ff!important}.rt-recording-stop{border:0!important;background:#fff!important;color:#651426!important}.rt-recording-banner:not(.verified-recording) .rt-recording-stop{display:none}.rt-recording-banner.verified-recording .rt-recording-auto,.rt-recording-banner.verified-recording .rt-recording-settings,.rt-recording-banner.starting .rt-recording-auto{display:none}.rt-recording-video,.rt-recording-weather{display:flex;align-items:center;gap:5px;font-size:11px;color:#bcd0e1;white-space:nowrap}.rt-recording-banner.verified-recording .rt-recording-video,.rt-recording-banner.starting .rt-recording-video,.rt-recording-banner.verified-recording .rt-recording-weather,.rt-recording-banner.starting .rt-recording-weather{display:none}
      body.rt-record-mode main{padding-bottom:calc(210px + var(--rt61-nav-h,72px) + env(safe-area-inset-bottom))!important}body.rt-record-mode main>.controls{display:none!important}body.rt-record-mode #rtCommunityPreflight{display:none!important}
      @media(max-width:640px){.rt-recording-banner{grid-template-columns:auto minmax(0,1fr);padding:11px}.rt-recording-actions{grid-column:1/-1;display:grid;grid-template-columns:auto auto minmax(0,1fr) auto}.rt-recording-actions button{min-height:46px}.rt-recording-video,.rt-recording-weather{justify-self:start}}
  ...
  function installRecordingBanner() {
    ...
    if (!banner) {
      ...
      let weatherEnabled=false;try{weatherEnabled=Boolean(JSON.parse(localStorage.getItem('rideTracker.contextPreferences.v1')||'{}').weatherEnabled)}catch(_){}
      banner.innerHTML = `<span class="rt-recording-dot"></span><div class="rt-recording-copy"><strong>Fahrt automatisch starten</strong><span>Initialisierung, Kalibrierung und Aufnahme erfolgen in einem Schritt.</span></div><div class="rt-recording-actions"><label class="rt-recording-video"><input type="checkbox" data-record-video checked> Video</label><label class="rt-recording-weather"><input type="checkbox" data-record-weather ${weatherEnabled?'checked':''}> Wetter</label><button type="button" class="rt-recording-auto">Automatisch starten</button><button type="button" class="rt-recording-settings">Sensoren</button><button type="button" class="rt-recording-stop">Stoppen</button></div>`;
