...
for (const token of ['fitHudStage','availableW','availableH','16 / 9','9 / 16','RideTrackerHudStageFit','visualViewport','maxByViewportW','maxByViewportH']) need(u50,token,`HUD viewport containment missing ${token}`);
...
for (const token of ['data-motion-live','stopImmediatePropagation','Berechtigung wird geprüft','button.isConnected']) need(u50,token,`Safari motion permission guard missing ${token}`);
...
if (fs.existsSync('index.html')) {
  ...
  if (built) {
    ...
    need(html,'update50.js?v=','Built page must load final HUD/sensor fixes');
