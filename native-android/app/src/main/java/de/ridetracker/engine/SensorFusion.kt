...
class AltitudeFusion(
    ...
    fun reset() { relativeAltitudeM = null; barometerZero = null; gpsBias = 0.0 }
...
class RidePhaseDetector {
    ...
    fun update(t: Double, speedMs: Double, longitudinalG: Double, climbRateMs: Double, totalG: Double): String {
        var next = phase
        when {
            ...
            speedMs > 0.8 || abs(totalG - 1) > 0.18 -> next = "ride"
                ...
                if (t - (stationarySince ?: t) >= 4) next = if (phase == "idle") "ready" else "station"
            }
        }
        if (speedMs > 0.8) stationarySince = null
        if (next != phase) { phase = next; events += t to next }
        return phase
    }
}

object QualityScore {
    fun calculate(motionSamples: Int, gpsAccepted: Int, gpsRejected: Int, gaps: Int, calibrated: Boolean, hasBarometer: Boolean): Int {
        ...
        val raw = 100 * (0.30 * motion + 0.30 * gpsRatio + 0.20 * if (calibrated) 1 else 0 + 0.10 * if (hasBarometer) 1 else 0 + 0.10) - 100 * penalty
