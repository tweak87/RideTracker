...
internal data class AndroidMetricBinding(var metric:String,var primarySource:String,var fallbackSources:MutableList<String>,var minimumQuality:Double,var maxAgeMs:Int,var interpolation:String,var widgetId:String?=null)
...
internal class AndroidDeviceRegistry(private val context:Context) {
    ...
    private fun loadDevices():MutableList<AndroidDeviceDescriptor> = runCatching {
        val raw=prefs.getString("devices",null) ?: return@runCatching defaults(); val array=JSONArray(raw)
        MutableList(array.length()){i->val d=array.getJSONObject(i);val ca=d.getJSONArray("channels");AndroidDeviceDescriptor(d.getString("id"),d.getString("name"),d.getString("type"),d.getString("transport"),d.getBoolean("enabled"),d.optBoolean("autoReconnect",true),MutableList(ca.length()){j->val c=ca.getJSONObject(j);AndroidDeviceChannel(c.getString("id"),c.getString("metric"),c.optString("unit"),c.getBoolean("enabled"),c.optDouble("sampleRateHz",1.0),c.optString("calibratedAt").ifBlank{null})})}
    }.getOrElse{defaults()}
    private fun loadBindings():MutableList<AndroidMetricBinding> = runCatching {
        val raw=prefs.getString("metricBindings",null) ?: return@runCatching defaultBindings(); val a=JSONArray(raw)
        MutableList(a.length()){i->val b=a.getJSONObject(i);val f=b.optJSONArray("fallbackSources")?:JSONArray();AndroidMetricBinding(b.getString("metric"),b.optString("primarySource"),MutableList(f.length()){j->f.getString(j)},b.optDouble("minimumQuality",0.6),b.optInt("maxAgeMs",1000),b.optString("interpolation","hold"),b.optString("widgetId").ifBlank{null})}
    }.getOrElse{defaultBindings()}
    fun save(){
        val a=JSONArray();devices.forEach{d->val channels=JSONArray();d.channels.forEach{c->channels.put(JSONObject().put("id",c.id).put("metric",c.metric).put("unit",c.unit).put("enabled",c.enabled).put("sampleRateHz",c.sampleRateHz).put("calibratedAt",c.calibratedAt?:""))};a.put(JSONObject().put("id",d.id).put("name",d.name).put("type",d.type).put("transport",d.transport).put("enabled",d.enabled).put("autoReconnect",d.autoReconnect).put("channels",channels))};prefs.edit().putString("devices",a.toString()).apply();devices=devices.toMutableList()
    }
    fun saveBindings(){val a=JSONArray();bindings.forEach{b->a.put(JSONObject().put("metric",b.metric).put("primarySource",b.primarySource).put("fallbackSources",JSONArray(b.fallbackSources)).put("minimumQuality",b.minimumQuality).put("maxAgeMs",b.maxAgeMs).put("interpolation",b.interpolation).put("widgetId",b.widgetId?:""))};prefs.edit().putString("metricBindings",a.toString()).apply();bindings=bindings.toMutableList()}
    ...
    fun sources(metric:String)=devices.flatMap{d->d.channels.filter{it.metric==metric||(metric=="gForce"&&it.metric=="acceleration")}.map{"${d.id}/${it.id}"}}
}

@Composable internal fun AndroidDeviceCenter(modifier:Modifier, registry:AndroidDeviceRegistry, heartRate:AndroidHeartRateManager){
    var showRouting by remember{mutableStateOf(false)}
    Column(modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(18.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
        Row{Text(if(showRouting)"Quellen & Prioritäten" else "Geräte & Sensoren",style=MaterialTheme.typography.headlineMedium,modifier=Modifier.weight(1f));if(showRouting)Button(onClick={showRouting=false}){Text("Zurück")}else Button(onClick=registry::add){Text("Hinzufügen")}}
        if(showRouting){
            registry.bindings.forEach{b->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text(b.metric,style=MaterialTheme.typography.titleMedium);var expanded by remember(b.metric){mutableStateOf(false)};Box{OutlinedButton(onClick={expanded=true}){Text("Primär: ${b.primarySource.ifBlank{"Keine"}}")} DropdownMenu(expanded=expanded,onDismissRequest={expanded=false}){DropdownMenuItem(text={Text("Keine")},onClick={b.primarySource="";registry.saveBindings();expanded=false});registry.sources(b.metric).forEach{s->DropdownMenuItem(text={Text(s)},onClick={b.primarySource=s;registry.saveBindings();expanded=false})}}};OutlinedTextField(b.fallbackSources.joinToString(", "),{b.fallbackSources=it.split(',').map(String::trim).filter(String::isNotEmpty).toMutableList();registry.saveBindings()},label={Text("Ersatzquellen")},modifier=Modifier.fillMaxWidth());Text("Mindestqualität: ${"%.2f".format(b.minimumQuality)}");Slider(b.minimumQuality,{b.minimumQuality=it;registry.saveBindings()},valueRange=0f..1f,steps=19);OutlinedTextField(b.maxAgeMs.toString(),{b.maxAgeMs=it.toIntOrNull()?:b.maxAgeMs;registry.saveBindings()},label={Text("Maximales Alter ms")},modifier=Modifier.fillMaxWidth());Row{listOf("none","hold","linear").forEach{mode->FilterChip(selected=b.interpolation==mode,onClick={b.interpolation=mode;registry.saveBindings()},label={Text(mode)});Spacer(Modifier.width(6.dp))}}}}}
        } else {
            Card{Column(Modifier.padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text("BLE Herzfrequenz",style=MaterialTheme.typography.titleMedium);Text(heartRate.status);heartRate.latestHeartRate?.let{Text("$it BPM")};Row{Button(onClick=heartRate::scan){Text("Suchen")};Spacer(Modifier.width(8.dp));Button(onClick=heartRate::connect){Text("Verbinden")}}}}
            Button(onClick={showRouting=true},modifier=Modifier.fillMaxWidth()){Text("Quellen & Prioritäten")}
            registry.devices.forEach{device->var expanded by remember(device.id){mutableStateOf(false)};Card(onClick={expanded=!expanded},modifier=Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text(device.name,style=MaterialTheme.typography.titleMedium);Text("${device.transport} · ${device.channels.size} Kanäle");if(expanded){OutlinedTextField(device.name,{device.name=it;registry.save()},label={Text("Name")},modifier=Modifier.fillMaxWidth());Row{Text("Aktiv",Modifier.weight(1f));Switch(device.enabled,{device.enabled=it;registry.save()})};Row{Text("Automatisch verbinden",Modifier.weight(1f));Switch(device.autoReconnect,{device.autoReconnect=it;registry.save()})};device.channels.forEach{channel->HorizontalDivider();Text(channel.metric,style=MaterialTheme.typography.titleSmall);Row{Text("Kanal aktiv",Modifier.weight(1f));Switch(channel.enabled,{channel.enabled=it;registry.save()})};OutlinedTextField(channel.sampleRateHz.toString(),{channel.sampleRateHz=it.toDoubleOrNull()?:channel.sampleRateHz;registry.save()},label={Text("Messrate Hz")},modifier=Modifier.fillMaxWidth());Button(onClick={channel.calibratedAt=java.time.Instant.now().toString();registry.save()}){Text(if(channel.calibratedAt==null)"Separat kalibrieren" else "Neu kalibrieren")}}}}}}
        }
    }
}
