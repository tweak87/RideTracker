            ...
            MaterialTheme {
                ...
                Column(Modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        Button(enabled = !recorder.isRecording, onClick = { permissionLauncher.launch(permissions) }) { Text("Initialisieren & Start") }
                        Button(enabled = recorder.isRecording, onClick = recorder::stop) { Text("Stop") }
                    }
                    Button(enabled = !recorder.isRecording && recorder.sampleCount > 0, onClick = { recorder.saveSession() }) {
