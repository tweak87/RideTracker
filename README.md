...
Die aktuelle Version `2026.08.08-ice-telemetry-fire8.3` ergänzt Web und Android um eine wählbare Parkkarte im Umkreis, optionale Wetter-/Wind-Snapshots, lizenzierte Fahrtbilder als Miniaturen, einen automatischen Aufnahme-Dialog und einen strengeren Stillstandfilter gegen GPS-Sprünge. Für Fahrten in abgeschirmten Fahrzeugen repariert die Web-App wiederholte Browser-GPS-Zeitstempel und kennzeichnet eine nicht belastbare Position ehrlich als unbekannt. Eine Lageprüfung verhindert die Wiederverwendung unpassender G-Kraft-Kalibrierungen; die neue Diagnose zeigt Seitenkraft zusätzlich in m/s² und als horizontalen Gesamtvektor. FAQ, Kompass und frei navigierbares XYZ-Modell mit Punktinspektor sind in beiden Oberflächen verfügbar. Das Supabase-Backend erhält versionierte Datenschutzzustimmung, serverseitige Synchronisierungssperre sowie Export und Löschung der eigenen Community-Daten.
...
Der Android-Workflow testet und prüft die App, verifiziert die Fire-APK zusätzlich mit `apksigner`, `aapt` und einem vollständigen Archivtest und veröffentlicht Builds von `main` als Vorab-Release unter [`android-v2026.08.08.3`](https://github.com/tweak87/RideTracker/releases/tag/android-v2026.08.08.3). Auf Fire OS 8 ausschließlich `INSTALL-RideTracker-FIRE-OS-8-v2026.08.08.3.apk` öffnen; im Installer muss **RideTracker FIRE 8 TEST** stehen. Die Test-APKs sind nicht für einen App-Store signiert; für wiederholte Updates und eine öffentliche Verteilung wird ein geschützter Release-Key benötigt. Fire-Hinweise stehen in [`docs/ANDROID_FIRE_INSTALL.md`](docs/ANDROID_FIRE_INSTALL.md). Der iOS-Workflow prüft den Swift-Code mit einem Simulator-Build; für die Installation auf einem realen iPhone ist ebenfalls Signierung erforderlich.
