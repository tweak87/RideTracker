...
enum NativeSection: String, CaseIterable, Identifiable {
    case home, record, rides, map, settings, hud
    var id: String { rawValue }
    var title: String {
        switch self {
        case .home: "Start"
        case .record: "Aufzeichnen"
        case .rides: "Fahrten"
        case .map: "Karte"
    ...
    var icon: String {
        ...
        case .hud: "rectangle.3.group.fill"
        }
    }
}
...
struct AppShellView: View {
    ...
    var body: some View {
        TabView(selection: $section) {
            NativeDashboard(section: $section, profileName: profiles.activeProfile.name, showProfiles: { showProfiles = true })
                .tag(NativeSection.home).tabItem { Label("Start", systemImage: NativeSection.home.icon) }
            NativeRecordingView()
                .tag(NativeSection.record).tabItem { Label("Aufzeichnen", systemImage: NativeSection.record.icon) }
            RideLibraryView()
                .tag(NativeSection.rides).tabItem { Label("Fahrten", systemImage: NativeSection.rides.icon) }
            RideMapListView()
            ...
            if recorder.isRecording {
                HStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading) {
                        ...
                        Text("Sensoren und optional Video werden aufgezeichnet.").font(.caption).foregroundStyle(.secondary)
                    }
                    Spacer()
                    Button("Stoppen", role: .destructive) { recorder.stop() }.buttonStyle(.borderedProminent).tint(.red)
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    Text("Angemeldet: \(profileName)").foregroundStyle(.secondary)
                    NativeMenuCard("Neue Fahrt", "Video und Telemetrie aufzeichnen", "record.circle.fill") { section = .record }
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 14) {
                    ...
                    HStack { Text("Höhe"); Spacer(); Text(String(format: "%.1f m", recorder.relativeAltitude)).monospacedDigit() }
                    ...
                    }.buttonStyle(.borderedProminent).tint(recorder.isRecording ? .red : .blue).frame(maxWidth: .infinity)
...
private struct NativeMenuCard: View {
    let title: String; let subtitle: String; let icon: String; let action: () -> Void
    init(_ title: String, _ subtitle: String, _ icon: String, action: @escaping () -> Void) { self.title = title; self.subtitle = subtitle; self.icon = icon; self.action = action }
    var body: some View {
        Button(action: action) {
            ...
            }.padding().background(.thinMaterial, in: RoundedRectangle(cornerRadius: 17))
        }.buttonStyle(.plain)
    }
}

private struct NativeProfileSheet: View {
    @EnvironmentObject private var profiles: UserProfileStore
            ...
            Form {
                Section("Profile") {
                    ForEach(profiles.profiles) { profile in
                        Button(profile.id == profiles.activeProfileID ? "✓ \(profile.name)" : profile.name) { profiles.select(profile.id) }
                    }
                }
                Section("Neu") {
                    TextField("Benutzername", text: $name)
                    Button("Profil anlegen") { profiles.create(name: name); name = "" }.disabled(name.trimmingCharacters(in: .whitespaces).isEmpty)
                }
            }.navigationTitle("Benutzer").toolbar { Button("Fertig") { dismiss() } }
        }
    }
}
